...
class ServiceController extends AppBaseController
{
	...
	public function Query()
	{
		try
		{
			...
			if ($filter) $criteria->AddFilter(
				new CriteriaFilter('Id,Name,Code,Description,CreatedDate,UpdatedDate,Status'
				, '%'.$filter.'%')
			);
	...
	public function Create()
	{
		try
		{
			...
			$service->Code = $this->SafeGetVal($json, 'code');
			$service->Description = $this->SafeGetVal($json, 'description');
			$service->CreatedDate = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'createdDate')));
			$service->UpdatedDate = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'updatedDate')));
			$service->Status = $this->SafeGetVal($json, 'status');
	...
	public function Update()
	{
		try
		{
			...
			$service->Code = $this->SafeGetVal($json, 'code', $service->Code);
			$service->Description = $this->SafeGetVal($json, 'description', $service->Description);
			$service->CreatedDate = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'createdDate', $service->CreatedDate)));
			$service->UpdatedDate = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'updatedDate', $service->UpdatedDate)));
			$service->Status = $this->SafeGetVal($json, 'status', $service->Status);
